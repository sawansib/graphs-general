...
#series = ['COOL']
#series = ['BASE_64', 'COOL_64'] 
#series = ['BaselineSMT', 'Manual', 'allDRF'] 
...
if y_axis_title.find("%") >= 0:
    ...
    legend_rows = 2
    legend_x = frame_x - 10 #++ moves right -- moves left #250 #90 #Change legend position from here
    legend_y = -2          #++ down --up
if len(series) < 4:
...
if len(series) > 1:
    ar = area.T(size = area_size,
                ...
                x_axis = axis.X(label = x_axis_title , format="/hR/12/a40 %s"),
...
for serie_i in range(len(series)):
    ...
    ar.add_plot(bar_plot.T(data = serie_data,
                           ...
                           #fill_style = rgb_fill_style_color[serie_i%len(rgb_fill_style_color)],
