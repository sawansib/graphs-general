...
# Plots 
# usage: plot-lines x_col data_col title? comment? y_axis_title? maxy_range? miny_range?
...
if len(args) >= 7 and args[6] != "":
    miny_range = float(args[6])

maxy_range = None
if len(args) >= 6 and args[5] != "":
    maxy_range = float(args[5])
...
line_style_array = (line_style.T(color=color.blue, width=0.8), line_style.T(color=color.brown, width=0.8), line_style.T(color=color.gold, width=0.8), line_style.T(color=color.green4, width=0.8), line_style.T(color=color.purple, width=0.8))
...
#            x_range = (min(series), max(series)),
            ...
            x_axis = axis.X(label="/b"+x_axis_title), #, format = format_angle),
            y_axis = axis.Y(label="/b"+y_axis_title, format = "%.1f%%"),
            legend = None)

x_data = [(r[x_col], r[data_col], r[data_col + 1]) for r in data]
x_data.sort (my_cmp)
