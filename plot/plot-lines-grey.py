...
if maxy_range - miny_range > 4:
    y_grid_interval = 0.5
elif maxy_range - miny_range > 2.5:
    y_grid_interval = 0.3
elif maxy_range - miny_range > 1.5:
    y_grid_interval = 0.2
elif maxy_range - miny_range > 0.2:
    y_grid_interval = 0.04
elif maxy_range - miny_range > 0.1:
...
line_style_array2 = (line_style.T(color=color.gray10, width=0.8), line_style.T(color=color.gray30, width=0.8, dash=(3,2)), line_style.T(color=color.gray40, width=0.8, dash=(2,2)), line_style.T(color=color.gray50, width=0.8, dash=(1,2)))
...
#            x_range = (min(series), max(series)),
            ...
            x_axis = axis.X(label = None), #, tic_interval = 2), #, format="/a90 %s"), # label="/b"+x_axis_title
            y_axis = axis.Y(label = y_axis_title, format = "%.2f", label_offset = (0,-8)), # label="/b"+y_axis_title
...
for legend_i in range(len(legend_tags)):
    ...
    ar.add_plot(line_plot.T(data = x_data,
                            ...
                            line_style = line_style_array2[legend_i%len(line_style_array2)],
